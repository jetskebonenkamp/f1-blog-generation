...
class PeftFinetuner:
    ...
    def saveParameters(self, model):
        ...
        for _, p in model.named_parameters():
            ap += p.numel()
            if p.requires_grad: tp += p.numel()
        pt = str((tp / ap) * 100) + '%'
        headers = ['Trainable parameters', 'All parameters', 'Perc. trainable']
        print('Total number of parameters:', ap)
        df = pd.DataFrame(data=[[tp, ap, pt]], columns=headers)
        df.to_csv('FinetuningStats.csv', index=False)
