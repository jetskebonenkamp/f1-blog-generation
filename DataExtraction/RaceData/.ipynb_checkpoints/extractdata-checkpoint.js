...
async function getZipContent(year, race_name) {
    ...
    fs.readFile(dt_path, 'utf8', (errorx, datax) => {
        ...
        // the content of the ZipFiles folder is not included in the
        // repository due to its excessive size
        const zip_file = "./ZipFiles/" + year + "_" + race_name + ".zip";
        const tt_folder = './TFeed_Timetables/' + year + '_' + race_name;
...
async function loadData() {
    ...
    for (var j = 2018; j < 2023; j++) {
        ...
        for (var k = 0; k < races_y.length; k++) {
            ...
            // saveSessionHistory(year, race_name);
