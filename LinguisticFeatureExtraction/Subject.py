...
class Subject:
    def __init__(self, verb, nouns, doc, coref):
        self.nouns, self.doc, self.coref = nouns, doc, coref
        ...
        else: self.full_subj, self.full_subj2 = None, None
    ...
    def getSubject(self, verb):
        ...
        if verb.head.dep_ == 'conj':
            for child in verb.head.children:
                if 'nsubj' in child.dep_: return child
        if verb.head.ent_type_ == 'DRIVER': return verb.head
        return None
    ...
    def additionalSubject(self):
        ...
        for child in subj.children:
            if child.dep_ == 'conj': return self.getFullSubject(child)
        return None

    def subjectList(self):
        # return a list of the subjects present in the blog
        if self.full_subj2: return [self.full_subj, self.full_subj2]
        else: return [self.full_subj]
